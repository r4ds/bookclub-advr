...
-   See how treating code as data enables domain-specific languages (DSLs)
...
# These are needed for actually running the code in the slides, but I'm showing them in theirs respective slide.
...
Basically, HTML tags are "ordinary" R functions

## Our DSL makes translation easy

-   **Same structure**: the nesting of function calls matches the nesting of tags.

-   **Similar logic**: unnamed arguments become the content of the tag, and named arguments become their attributes.

-   **User experience**: special characters are automatically escaped (e.g. `&`).

## We need to know five basic HTML tags for this exercise
...
## We are going to create HTML DSL following a micro-macro procedure
...
-   Create an S3 that translates and automatically escape user's input
-   Create a basic structure for tag functions
-   Create a function factory for creating tag functions
-   Create and HTML environment of evaluation
...
## We can create HTML tags creating corresponding R functions

-   Let's create `<p>` tag by creating `p()` function, then we use this logic to create the other tags.
...
## We need to create and HTML environment of evaluation
...
## Creating a list makes translation works
...
## We are going to create LaTeX DSL following a macro-micro procedure
...
## Our HTML DSL is going to be different than our LaTeX DSL
...
We need to leave non-Greek symbols as they are, but we don´t know what symbols are going to be used.
...
## Is possible to R → HTML via a micro-macro procedure
...
## Is possible R → LaTeX via macro-micro procedure
...
-   Integranting all the environment in the dynamic environment contained in the `to_math()` interface
