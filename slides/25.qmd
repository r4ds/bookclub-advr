...
-   Why rewrite R code
-   Bridge R with C++/Rust
-   Run C++ in R
-   Run Rust in R
-   Create and call C++ functions in R
-   Create and call Rust functions in R
...
-   R or C code debugging is more challenging than Rust
    ...
    - Rust has no GC so lacks memory bugs and has great error messages surpassing(?) the {tidyverse}
...
## The C optimized `sign` estsablishes the baseline performance to compare against
...
## C++ re-implementation is faster but uses more memory than Rust
...
-   [`{rextendr}`](https://extendr.rs/rextendr/index.html)
-   [Rust for Developers](https://extendr.rs/intro-rust/)
