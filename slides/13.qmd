...
## Create a new object ~ create an S3 class
...
## It is recommended S3 classes be provided with 3 functions
...
## The custom `summarizer` S3 class has the 3 recommended functions
...
```{r}
new_summarizer <- function(x) {
  stopifnot(is.numeric(x) || is.character(x))
  structure(x, class = "summarizer")
}
```
