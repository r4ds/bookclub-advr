...
Learn (more deeply?) the **tools** for controlling flow:

- Choices (`if`, `switch`, `ifelse`, `dplyr::if_else`, `dplyr::case_when`)
- Loops (`for`, `while`)
...
This chapter is about flow control, including some technical details and useful features. 

There are two main groups of flow control tools: **choices** and **loops**. These are both very useful for data analysis where iteration cannot be avoided with other (e.g. functional tools)  
