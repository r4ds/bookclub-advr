...
A close look at C++ will provide an overview of the language and its key conventions to focus on the differences with R. We will also have look at the standard template library STL, the C++ library to use, which provides a set of extremely useful data structures and algorithms.
...
Call the files: - in R use `source(echo = TRUE)` - in C++ use `sourceCpp("path/to/file.cpp")`
...
Actions to convert R to C++: - Add type declarations to all variables - Use `(` instead of `[` to index into the matrix - Subscript the results of `rgamma` and `rnorm` to convert from a vector into a scalar.
